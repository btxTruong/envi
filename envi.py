...
def _get_necessary_meaning(all_meaning):
    unnecessary_meaning = re.compile(r'Cấu trúc từ|Hình Thái Từ', re.IGNORECASE)

    return [mean for mean in all_meaning if not unnecessary_meaning.search(mean)]
...
def envi(word):
    ...
    if mean:
        ...
        necessary_meaning = _get_necessary_meaning(all_meaning)
